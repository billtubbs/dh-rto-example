...
def plot_dispatch(
    ...
    ax.bar(
        x + width / 2,
        -dispatch["storage_charge"].to_numpy(),
        width=width,
    ...
    ax2.fill_between(
        x_storage, storage.to_numpy(), color="#FDD835", alpha=0.4, **step_kw
    )
    ax2.plot(
        x_storage,
        storage.to_numpy(),
    ...
    ax2.set_xticklabels(
        [dispatch.index[i].strftime(date_fmt) for i in x[::step]],
        rotation=45,
        ha="right",
    )
...
if __name__ == "__main__":
    dispatch_rto = pd.read_csv(
        RESULTS_DIR / "rto_hourly_dispatch.csv", index_col=0, parse_dates=True
    )
    storage_rto = pd.read_csv(
        RESULTS_DIR / "rto_hourly_storage.csv", index_col=0, parse_dates=True
    ).iloc[:, 0]
    dispatch_pf = pd.read_csv(
        RESULTS_DIR / "perfect_foresight_dispatch.csv",
        index_col=0,
        parse_dates=True,
    )
    storage_pf = pd.read_csv(
        RESULTS_DIR / "perfect_foresight_storage.csv",
        index_col=0,
        parse_dates=True,
    ).iloc[:, 0]
    ...
    supply_rto = (
        w_rto["gas_boiler"] + w_rto["heat_pump"] + w_rto["storage_discharge"]
    )
    supply_pf = (
        w_pf["gas_boiler"] + w_pf["heat_pump"] + w_pf["storage_discharge"]
    )
    top = (
        max(
            w_rto["heat_demand"].max(),
            w_pf["heat_demand"].max(),
            supply_rto.max(),
            supply_pf.max(),
        )
        * 1.45
    ...
    max_charge = max(
        w_rto["storage_charge"].max(), w_pf["storage_charge"].max()
    )
    ...
    plot_dispatch(
        dispatch_rto,
        storage_rto,
        "RTO (hourly) dispatch around cold snap",
        PLOT_DIR / "rto_coldsnap.png",
        ZOOM_START,
        ZOOM_HOURS,
        freq="h",
        ylim_dispatch=ylim_dispatch_hourly,
        ylim_storage=ylim_storage,
    )
    plot_dispatch(
        dispatch_pf,
        storage_pf,
        "Perfect foresight dispatch around cold snap",
        PLOT_DIR / "perfect_foresight_coldsnap.png",
        ZOOM_START,
        ZOOM_HOURS,
        freq="h",
        ylim_dispatch=ylim_dispatch_hourly,
    ...
    daily_dispatch_rto, daily_storage_rto = aggregate_daily(
        dispatch_rto, storage_rto
    )
    daily_dispatch_pf, daily_storage_pf = aggregate_daily(
        dispatch_pf, storage_pf
    )

    daily_supply_rto = (
        daily_dispatch_rto["gas_boiler"]
        + daily_dispatch_rto["heat_pump"]
        + daily_dispatch_rto["storage_discharge"]
    )
    daily_supply_pf = (
        daily_dispatch_pf["gas_boiler"]
        + daily_dispatch_pf["heat_pump"]
        + daily_dispatch_pf["storage_discharge"]
    )
    daily_top = (
        max(
            daily_dispatch_rto["heat_demand"].max(),
            daily_dispatch_pf["heat_demand"].max(),
            daily_supply_rto.max(),
            daily_supply_pf.max(),
        )
        * 1.45
    )
    daily_max_charge = max(
        daily_dispatch_rto["storage_charge"].max(),
        daily_dispatch_pf["storage_charge"].max(),
    )
    ylim_dispatch_daily = (
        -daily_max_charge * 1.15 if daily_max_charge > 0 else -1,
        daily_top,
    )
    ...
    plot_dispatch(
        daily_dispatch_rto,
        daily_storage_rto,
        "RTO (hourly) dispatch -- daily totals",
        PLOT_DIR / "rto_daily.png",
        zoom_start=None,
        freq="D",
        ylim_dispatch=ylim_dispatch_daily,
        ylim_storage=ylim_storage,
    )
    plot_dispatch(
        daily_dispatch_pf,
        daily_storage_pf,
        "Perfect foresight dispatch -- daily totals",
        PLOT_DIR / "perfect_foresight_daily.png",
        zoom_start=None,
        freq="D",
        ylim_dispatch=ylim_dispatch_daily,
        ylim_storage=ylim_storage,
